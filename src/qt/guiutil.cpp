...
{
    // Convert vrscoin:// to vrscoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("vrscoin://"))
    {
        uri.replace(0, 12, "vrscoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "VRSCoin.lnk";
}
...
{
    return GetAutostartDir() / "vrscoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("VRSCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  vrscoin-qt [" + tr("command-line options") + "]                     " + "\n";
